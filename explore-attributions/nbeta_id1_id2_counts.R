# Count samples by InsDel_N_beta x InsDel1{a/b/c} x InsDel2{a/b/c} presence
# pattern, broken down by Major Cancer Type (MSS non-hypermutated only),
# plus MSI-H and MSS-hypermutated columns. Presence = share >= 5%.
#
# Rows (samples with InsDel_N_beta present, partitioned):
#   1. N_beta only (no InsDel1, no InsDel2)
#   2. N_beta + >=1 InsDel1 + 0 InsDel2
#   3. N_beta + 0 InsDel1 + >=1 InsDel2
#   4. N_beta + >=1 InsDel1 + >=1 InsDel2
#
# Columns: Major Cancer Types (MSS non-hyper), MSI-H, MSS hypermutated, Row total.
# No double counting across strata.
...
# Column scheme: Major Cancer Types (MSS non-hyper only), then MSI-H, MSS hyper, then Row total
type_order <- sort(unique(samp$major[!is.na(samp$major)]))

with_cat <- samp[!is.na(samp$category) & !is.na(samp$major), ]

# Cancer-type columns: MSS non-hyper samples only
...
# Build full table: rows in fixed order, all type columns present
...
write.csv(full,
          file.path(this_dir, "nbeta_id1_id2_counts.csv"),
          row.names = FALSE)
message("Wrote nbeta_id1_id2_counts.csv")

# Pretty print
cat("\n=== Sample counts: InsDel_N_beta x InsDel1{a/b/c} x InsDel2{a/b/c} ===\n")
cat("(Cancer-type columns = MSS non-hypermutated samples; MSI-H and MSS hypermutated as separate columns)\n\n")
print(full, row.names = FALSE)

cat("\nN_beta-present sample totals by stratum:\n")
print(table(with_cat$stratum))
