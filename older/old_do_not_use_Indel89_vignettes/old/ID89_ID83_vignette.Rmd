---
title: "Indel89 signature vignettes"
output:
...
xplot89 <- function(ID89.catalog, text_size = 3, plot_title = "ID89",setyaxis=NULL,ylabel="Counts") {
...
ID89_ID83_connection_example <-
  # data.table::fread("~/Working/DBS_Indel_Signature/ID89_ID83_connection_example.txt")
  data.table::fread("./ID89_ID83_connection_example.txt")
colnames(ID89_ID83_connection_example) <- c("ID89_signature","example_catalog","ID83_signature")

to.plot.WGS_PCAWG_HMF_indels.catalog <- ICAMS::ReadCatalog("./PCAWG_HMF_ID83_catalog.tsv")
...
to.plot.all.signatures.final <- ICAMS::ReadCatalog("./all.ID83.signatures.tsv",catalog.type = "counts.signature")
to.plot.all.ID89.signatures.final <- as.data.frame(data.table::fread("./all.ID89.signatures.tsv"))
...
to.plot.all.ID89.catalogs <- as.data.frame(data.table::fread("./PCAWG_HMF_ID89_catalog.tsv"))
...
ID89.mSigAct.assignment <- readRDS("./PCAWG_HMF_ID89.mSigAct.PASA.rds")
```
...
for (i in c(1:42)) {
  ...
  p1 <- xplot89(to.plot.all.ID89.signatures.final[,ID89signature,drop=F],text_size = 3,plot_title = ID89signature,ylabel="Props")
  ...
  p4 <- xplot89(diff.catalog,text_size = 3,plot_title = paste0("Mutations due to ", ID89signature, " (A minus B)| Cosine similarity to ", ID89signature, " = :", cosine2))
...
cat(paste0("\n\n### ", ID89signature, " \n InsDel39 has no corresponding Indel83 signature \n\n  Supporting spectrum (two Indel83 spectrum show with and without deletion and insertions in long poly T tracts w): ", catalog, "\n\n"))
...
p3 <- xplot89(reconstructed.ID89.catalog,text_size = 3,
              plot_title = paste0("Spectrum B: Partial mutational spectrum of ",
                                  catalog, " not using ",ID89signature),
              setyaxis = max(reconstructed.ID89.catalog))

p4 <- xplot89(diff.catalog,text_size = 3,
              plot_title = paste0("Mutations due to ", ID89signature, " (A minus B)| Cosine similarity to ",
